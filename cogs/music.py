...
class LavalinkVoiceClient(discord.VoiceProtocol):
    ...
    def __init__(self, client: discord.Client, channel: discord.abc.Connectable):
        ...
        if not hasattr(self.client, 'lavalink'):
            ...
            self.client.lavalink.add_node(host='localhost', port=2555, password='youshallnotpass',
...
class Music(commands.Cog):
    def __init__(self, bot):
        ...
        if not hasattr(bot, 'lavalink'):
            ...
            bot.lavalink.add_node(host='localhost', port=2555, password='youshallnotpass',
                                  region='us', name='default-node')
    ...
    async def play(self, ctx, *, query: str):
        ...
        if results.load_type == LoadType.EMPTY:
            return await ctx.send("No tracks for that search were found.")
            ...
            embed.set_thumbnail(url=None or tracks[0].artwork_url)
        else:
            track = results.tracks[0]
            embed.title = 'Track Enqueued'
            embed.description = f'[{track.title}]({track.uri})'
            embed.set_thumbnail(url=None or results.tracks[0].artwork_url)
    ...
    @commands.check(create_player)
    async def loop(self, ctx, *, state: int = -1):

        player = self.bot.lavalink.player_manager.get(ctx.guild.id)

        embed = discord.Embed(color=discord.Color.blurple(), title="Loop Status")
        if(state == 0 or state == 1 or state == 2):
            player.set_loop(state)
            if(player.loop == player.LOOP_NONE):
                embed.description = 'Player updated to not loop.'
            elif(player.loop == player.LOOP_SINGLE):
                embed.description = (f'{player.current.title} will now loop!')
            elif(player.loop == player.LOOP_QUEUE):
                embed.description = ('Looping the entire queue.')
        else:
            if(player.loop == player.LOOP_NONE):
                embed.description = 'Currently the player is not looping any songs. Loop setting can be updated with the inputs below:\n'
            elif(player.loop == player.LOOP_SINGLE):
                embed.description = (f'Currently looping {player.current.title} forever. Loop setting can be updated with the inputs below:\n')
            elif(player.loop == player.LOOP_QUEUE):
                embed.description = ('Currently looping the entire queue. Loop setting can be updated with the inputs below:\n')
            embed.add_field(name="0",value="Tells the player to not loop songs.",inline=False)
            embed.add_field(name="1",value="Tells the player to loop the current track.",inline=False)
            embed.add_field(name="2",value="Tells the player to loop the entire queue.",inline=False)
        await ctx.send(embed=embed)
